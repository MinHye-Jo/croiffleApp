...
const MenuManagement = ({ route }) => {
  ...
  const menuSoldOutApi = async (type) => {
    ...
    if (type == 'sale') {
      ...
      if (data.return_code == 200) {
        setMenuData(
          menuData.map(o => {
            if (o.menuId == bodyData.menuId) o.soldout = "0";
            return o;
          })
        );
      }
      ...
      if (data.return_code == 200) {
        setMenuData(
          menuData.map(o => {
            if (o.menuId == bodyData.menuId) o.soldout = "1";
            return o;
          })
        );
      }
  ...
  return (
    <ScrollView style={styles.topContainer}>
      <DefaultActionModal modalOpen={modalOpen}
        onClose={() => setModalOpen(false)}
        onAction={() => menuSoldOutApi('')}
        title={modalTitle} modalText={modalText} modalTextSec={modalTextSec} />
