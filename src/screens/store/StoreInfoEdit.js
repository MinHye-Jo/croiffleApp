    ...
    if (data.return_code == 200) {
      setShopInfo({
        ...Object.keys(shopInfo).reduce((obj, key) => {
          if (key == 'file') {
            setFileName(data.response.shopImgUrl);
          } else if (key == 'phone') {
            obj[key] = data.response[key].replace(/(\d{3})(-*)(\d{4})(-*)(\d{4})/, '$1-$3-$5');
          } else {
    ...
    <ScrollView style={styles.topContainer} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ paddingLeft: 20, paddingRight: 20 }}>
        ...
        <View style={styles.row}>
          ...
          <TouchableOpacity
            style={{ ...styles.blueBtn, flex: 1, marginLeft: 10 }}
        ...
        <TextInput
          style={{ ...styles.greyInput, marginTop: 10 }}
